...
export async function buildAdvisorReply(message: string): Promise<string> {
  ...
  if (!trimmed || /مرحبا|السلام|hello|hi|hey|هلا/i.test(trimmed)) {
    return `أهلاً بكِ في VELORA 💜 أنا مستشارة الجمال الخاصة بكِ.

أخبريني عن بشرتكِ أو هدفكِ (ترطيب، إشراقة، حبوب، شعر، مكياج…) وسأبني لكِ اقتراحاً من منتجات متجرنا فقط.

يمكنكِ البدء بجملة مثل: «بشرتي جافة وباهتة» أو «أريد روتين صباحي بسيط».`;
  }
